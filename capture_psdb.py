...
import os
import sys
from detect.detector_capture import Detector
...
if __name__ == '__main__':
    ...
    while True:
        ...
        if video == None:
            filename = '1_detect.mp4'
            if os.path.isfile(filename):
                os.remove(filename)
            height, width = img.shape[:2] 
            video = cv2.VideoWriter(filename, -1, 29.97, (width,height))

        dets = detector.im_detect(img)[0]
        showImage(img, dets, 0.3)
